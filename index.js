      ...
      for (const s of dataSeats.sdata) {
        ...
        for (const session of s.data_session) {
          if (new Date(session.session_time).getMinutes() === 0) {
            messageMovieTimeSeats += `${new Date(session.session_time).getHours()}:${new Date(session.session_time).getMinutes()}0&nbsp;剩餘座位${
              session.left_seats
            }\n`
